...
function buildFwJs(config, licenseUid) {
  ...
  async function checkLicense() {
    try {
      const version = window.__TANA_PC_VERSION || 'fw';
      const url =
        LICENSE.endpoint +
  ...
  const nz  = (s) =>
    String(s === undefined || s === null ? '' : s).trim() !== '';
  ...
  function judgeText(scan, base, op, label) {
    ...
    switch (op) {
      ...
      case '後方一致':
      case '後部一致':
        ok = s.lastIndexOf(b) === s.length - b.length;
        break;
      default:
        return { specified, ok: true, reason: null };
    }
  ...
  function judgeNumber(scanNum, baseNum, op, label) {
    ...
    if (Number.isNaN(s)) {
      return {
        specified,
        ok: false,
        reason: label + ':' + op + ' (scan:NaN, base:' + b + ')'
      };
    }
    let ok = true;
    switch (op) {
  ...
  function judgeTime(scanMin, baseTimeStr, op, label) {
    ...
    const s =
      scanMin === undefined || scanMin === null ? null : scanMin;
    ...
    return {
      ...
      ok,
      reason:
        ok ? null : label + ':' + op + ' (scan:' + s + ', base:' + b + ')'
    };
  }
  ...
  function evaluateFieldConditions(rec, parsed, fieldDef) {
    ...
    for (let i = 0; i < maxConds - 1; i++) {
      ...
      const joinRaw = joinCode
        ? String(val(rec, joinCode) || '').trim().toLowerCase()
        : '';
  ...
  function parseScan(raw) {
    ...
    let minLen = 0;
    for (let i = 0; i < fields.length; i++) {
      const len = Number(fields[i].length) || 0;
      minLen += len;
    }
  ...
  function appendRow(appRec, parsed, evalRes) {
    ...
    if (!rec[tblCfg.code]) {
      rec[tblCfg.code] = { type: 'SUBTABLE', value: [] };
    }
  ...
  function buildScanUI() {
    ...
    if (!mount) {
      ...
      mount.id = 'tana-scan-fixed-fallback';
    ...
    input.type = 'text';
    input.style.cssText =
      'flex:1 1 auto; padding:6px 8px; border:1px solid #ccc; border-radius:6px;';
    ...
    row2.textContent =
      '固定長の SCAN 文字列を入力して Enter（例: 4文字+3文字+16文字+…）';
