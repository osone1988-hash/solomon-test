...
function initKv() {
  ...
  const output = $("kv-code-output");
    ...
    generateBtn.addEventListener('click', async () => {
      setBusy(true);
      try {
        const user = window.tanaAuth && window.tanaAuth.currentUser;
        if (!user || !user.uid) {
          showStatus(statusEl, 'ログイン情報を取得できません (uid 不明)', true);
          return;
        }

        const cfg = collectKvConfigFromUI();
        const jsCode = buildKvJs(cfg, user.uid);
        $('kv-code-output').value = jsCode;

        showStatus(
          statusEl,
          'JSコードを生成しました (ライセンス uid: ' + user.uid + ')',
          false
        );
      } catch (e) {
        console.error(e);
        showStatus(statusEl, 'エラー: ' + e.message, true);
      } finally {
        setBusy(false);
      }
    });
