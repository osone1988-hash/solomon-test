/* TANA-OROSHI pc.js — scan_area固定 / レコード番号に依存しない描画 / 数値の負数・全角・カンマ許容
   リロード無しで追記・複製防止 / 複数理由出力
   v=pc-ng-rules-2025-11-10-14 */
(function () {
  'use strict';
  const VERSION = 'pc-ng-rules-2025-11-10-14';
  try { console.log('[TANA-OROSHI] pc.js loaded:', VERSION); window.__TANA_PC_VERSION = VERSION; } catch(_) {}

  /* ==== 判定に使うフィールドコード（必要ならここを書き換え） ==== */
  const JUDGE = {
  ...
  function parseDateLoose(s){
    ...
    if (/^\d{8}$/.test(t)) {            // 20251109
      ...
      const dt=new Date(y,m,d); return Number.isNaN(dt.getTime())?null:dt;
    }
    const dt=new Date(t.replace(/\//g,'-'));  // 2025/11/09 09:00 など
    return Number.isNaN(dt.getTime()) ? null : dt;
  }

  // OK条件を満たさなければ NG（テキスト/数値/日時）
  ...
  const ok_number=(scan, label, base)=>{
    ...
    if (s==null || b==null) return false; // 値不正はNG
  ...
  async function putRowAndRefreshView(appId, recId, rowValueOnly){
    ...
    kintone.app.record.set({ record: res2.record }); // ← リロード無しで反映
  }

  // 入力: 文字 数値 日時…以降は結合（例: "2025-11-09 09:00"）
  function parseScan3(raw){
    const arr=S(raw).trim().split(/\s+/).filter(Boolean);
    const at = arr.shift() || '';
    const bt = arr.length ? toNumOrNull(arr[0]) : null; if (arr.length) arr.shift();
    const ct = arr.length ? parseDateLoose(arr.join(' ')) : null;
    return { at, bt, ct };
  }

  // ---- SCAN UI（どのレコード番号でも必ず描画）----
  function ensureScanUI(){
    ...
    // 優先2: テーブルの直前
    const tblEl = kintone.app.record.getFieldElement?.(TABLE);
    if (tblEl && tblEl.parentElement && tblEl.parentElement.parentElement) {
      const block = tblEl.parentElement.parentElement;
  ...
  function bindHandlersOnce(){
    if (window.__TANA_SCAN_BOUND__) return;
    ...
    document.addEventListener('keydown', async (ev)=>{
      ...
      try{
        ...
        // 判定
        const okA = ok_text(atTxt, aOp, bOp ? aVal : aVal);  // a はそのまま
        ...
        ip.value='';
        await putRowAndRefreshView(appId, recId, row);       // ← リロード無しで即反映
